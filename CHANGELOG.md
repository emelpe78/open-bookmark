...
- **Composable-Sync-Protokoll:** Tag-/Listen-Mutationen (Umbenennen, Löschen) aktualisieren automatisch die aktive Bookmark-Ansicht und Filter — keine manuellen `refreshViews()`-Aufrufe mehr in `useTagAdmin`/`useListAdmin` (#6).
- **Code-Konsolidierung:** `server/utils/tags.ts` in ihren einzelnen Aufrufer zurückgezogen (#5); `useBookmarkForm`-Shallow-Composable entfernt und Helper in die Form-Komponenten inline eingefügt (#4); `parseTagInput` + `normalizeTagName` in ein gemeinsames Shared-Module extrahiert, das von Nuxt-App und Chrome Extension geteilt wird (#3).
...
[1.0.2]: https://github.com/emelpe78/open-bookmark/releases/tag/v1.0.2
